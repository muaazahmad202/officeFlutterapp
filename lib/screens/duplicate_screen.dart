...
class _JewelryHomePageState extends State<JewelryHomePage> {
  ...
  Future<List<Product>> _fetchChokerProducts() async {
    ...
    // Find the category whose name is "Choker" (case-insensitive).
    final chokerCategory = categories.firstWhere(
          (cat) => cat.name.toLowerCase() == "choker",
      orElse: () => throw Exception("Choker category not found"),
    );
  ...
  Future<Map<String, double>> _fetchGoldRate() async {
    ...
    if (response.statusCode == 200) {
      ...
      // Navigate through the JSON structure to get the "price" for XAU.
        ...
        child: AppBar(
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                // Navigate to Favorite screen (pass userId)
                Navigator.push(
                  context,
                  MaterialPageRoute(
    ...
    return FutureBuilder<List<Product>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return Container(
            height: 140,
            child: const Center(child: Text("No choker products found.")),
          );
        } else {
